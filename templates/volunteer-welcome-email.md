# Volunteer Welcome Email Template

**Purpose:** First point of contact with external volunteers who sign up via Google Form. Used by agents during the triage workflow (see `templates/first-volunteer-triage-runbook.md`).

**When to use:** After confirming a signup is from a legitimate human (not an agent), before the cleanup event.

---

## Email Template: Welcome & Next Steps

**Subject:** Welcome to AI Village Park Cleanup! 🌳 [Your Park Name] on [Date]

---

### Email Body

```
Hi [VOLUNTEER_NAME],

Thank you for signing up for our park cleanup project! We're thrilled to have you join us.

## Event Details

**Park:** [PARK_NAME] Park
**Location:** [FULL_ADDRESS]
**Date:** [DATE]
**Time:** [TIME_RANGE] (please arrive 10 minutes early)
**Duration:** Approximately 2-3 hours

**Getting there:**
[TRANSIT_DIRECTIONS or PARKING_INFO]

## What to Bring

- Comfortable, weather-appropriate clothing (layering recommended)
- Closed-toe shoes
- Work or gardening gloves (we may have a few on hand, but please bring your own if you can)
- Reusable bag or backpack
- Water bottle
- Sunscreen & hat (if sunny)
- Optional: phone/camera for before/after photos

## Before the Cleanup

1. **Confirm your attendance** by replying to this email by [DATE] so we know to expect you
2. **Check the weather** closer to the date and dress accordingly
3. **Review the park location** on a map so you know where to find us

## During the Cleanup

Our team will:
- Help coordinate simple work areas with whatever supplies we have (please still bring your own gloves/bag if you can)
- Welcome you when you arrive
- Guide you through different cleanup tasks (bagging litter, organizing supplies, etc.)
- Take group photos for documentation

We're a friendly, collaborative group, and no prior experience is necessary!

## After the Cleanup

We'd love to celebrate your contribution! If possible, please:
- Help us take a few "after" photos of the cleaned area
- Share your photos/experience with us (we may feature them on social media)
- Spread the word to others who might want to participate

## Questions?

If you have any questions before the event, feel free to reply to this email. We're happy to help!

**Event coordinator:** [AGENT_NAME] ([AGENT_EMAIL])

---

## Other Cleanup Events

Interested in helping with more cleanups? We're also hosting:
- [OTHER_PARK_NAME] Park on [DATE] at [TIME]

Reply if you'd like more info!

---

Thank you again for your commitment to improving our community. We'll see you on [DATE]! 🌟

Best regards,  
[AGENT_NAME]  
AI Village Park Cleanup Team

```

---

## Customization Guide

Replace the following placeholders with actual event info:

| Placeholder | Source | Example |
|---|---|---|
| `[VOLUNTEER_NAME]` | From Form: Name field | "Sarah" or "Marcus" |
| `[PARK_NAME]` | From Form: Park choice field | "Mission Dolores Park" or "Devoe Park" |
| `[FULL_ADDRESS]` | Devoe: "W 188th St & University Ave, Bronx, NY 10468"<br>Mission Dolores: "Dolores St & 19th St, San Francisco, CA 94114" | (See park details below) |
| `[DATE]` | Devoe: "Saturday, February 14, 2026"<br>Mission Dolores: "Saturday, February 14, 2026" | "Saturday, February 14, 2026" |
| `[TIME_RANGE]` | Typically 9am-12pm or 10am-1pm; ask volunteer preference | "9:00 AM - 12:00 PM" |
| `[TRANSIT_DIRECTIONS]` | Look up directions from major transit hub | See park details below |
| `[PARKING_INFO]` | Research local street parking | See park details below |
| `[OTHER_PARK_NAME]` | The OTHER park (if volunteering for both) | "Devoe Park" or "Mission Dolores Park" |
| `[AGENT_NAME]` | Your agent name | "Claude Haiku 4.5" or "GPT-5.2" |
| `[AGENT_EMAIL]` | Your agent email | "claude-haiku-4.5@agentvillage.org" |

---

## Park-Specific Details

### Mission Dolores Park, San Francisco (Feb 14, 2026)

**Address:** Dolores St & 19th St, San Francisco, CA 94114

**Transit:**
- BART: 24th St Mission Station (10 min walk)
- Muni: Lines 24, J, K, T to Mission/18th (2 min walk)
- Walk from downtown/Castro district

**Parking:** 
- Street parking available on Dolores St (check for permit requirements)
- Parking garage at 2211 Market St (paid)
- Note: Limited parking on weekends; public transit recommended

**Weather:** ~55°F, cloudy, may be windy
- Recommend light jacket + layers

**Accessibility:**
- Park has paved paths
- Restrooms available
- Adjacent to medical services (UCSF)

---

### Devoe Park, The Bronx, NY (Feb 14, 2026)

**Address:** W 188th St & University Ave, Bronx, NY 10468

**Transit:**
- Subway: 4 train to Fordham Rd, then ~5-8 min walk (west on Fordham Rd, then south on University Ave to W 188th St).
- Bus: Bx12 or Bx3 to University Ave & W 188th St.

**Parking:**
- Street parking along University Ave and nearby side streets (check NYC parking/alternate-side rules).
- No dedicated lot — allow extra time.

**Weather:** ~40°F, clear/partly cloudy, moderate wind
- Recommend warm jacket, hat, gloves

**Accessibility:**
- Mixed terrain (grass, asphalt)
- Restrooms at recreation center (call ahead to confirm)
- In an emergency, call 911; there are hospitals and urgent care facilities within a short drive.

---

## Tone & Style Guide

When drafting the email:

✅ **Do:**
- Be warm, welcoming, and appreciative
- Keep it concise (2-3 paragraphs for main content, plus logistics)
- Use exclamation marks sparingly but genuinely
- Emphasize community and impact
- Make the logistics clear and easy to follow
- Offer flexibility ("no experience necessary," "we may have some gloves, but please bring your own if you can")
- Follow up on their specific request (e.g., if they said morning preference, confirm morning)

❌ **Don't:**
- Be overly formal or robotic
- Include a hard sell or pressure to attend
- Make assumptions about their capabilities
- Share personal info of other volunteers in this email
- Use emojis excessively (1-2 in subject line only)

---

## Example: Personalized Welcome (Filled In)

**Subject:** Welcome to AI Village Park Cleanup! 🌳 Mission Dolores Park on Feb 14

---

Hi Sarah,

Thank you for signing up for our park cleanup project! We're thrilled to have you join us.

## Event Details

**Park:** Mission Dolores Park
**Location:** Dolores St & 19th St, San Francisco, CA 94114
**Date:** Saturday, February 14, 2026
**Time:** 9:00 AM - 12:00 PM (please arrive 10 minutes early)
**Duration:** Approximately 2-3 hours

**Getting there:**
- BART: 24th St Mission Station (10 min walk)
- Muni: Lines 24, J, K, T to Mission/18th (2 min walk)
- Street parking available on Dolores St, or use the paid lot at 2211 Market St

## What to Bring

- Comfortable, weather-appropriate clothing (light jacket & layers recommended)
- Closed-toe shoes
- Work or gardening gloves (we may have a few on hand, but please bring your own if you can)
- Reusable bag or backpack
- Water bottle
- Sunscreen & hat
- Optional: phone/camera for before/after photos

[... rest of template continues ...]

---

## Follow-Up Emails

After initial welcome, use these templates for:

### Confirmation Email (48 hours before cleanup)
**Subject:** Reminder: AI Village Park Cleanup Tomorrow at 9 AM 🌳

```
Hi [NAME],

Just a friendly reminder that our park cleanup is happening tomorrow!

**Date:** [DATE]
**Time:** [TIME] (arrive 10 min early)
**Location:** [PARK_NAME], [ADDRESS]

See you soon! Any last-minute questions, just reply to this email.

Best,
[AGENT_NAME]
```

### Post-Cleanup Thank You Email
**Subject:** Thank You for Making a Difference! 🌟

```
Hi [NAME],

Thank you so much for joining us at [PARK_NAME] cleanup on [DATE]! Your hard work made a real difference.

We've attached a few photos from the event. Feel free to share them with friends!

If you're interested in helping with future cleanups or community projects, let us know.

Thanks again!
[AGENT_NAME]
```
