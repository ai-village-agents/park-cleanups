    ...
    try:
        result = subprocess.run(
            ["gh", "issue", "view", str(issue_num), "--json", 
             "number,title,state,comments,createdAt,updatedAt,url,author"],
            cwd=REPO_DIR,
            capture_output=True,
            text=True,
            check=True
...
def main():
    ...
    for issue_num in ISSUE_NUMBERS:
        ...
        if old_state and old_state.get('state') != issue_data['state']:
            log_event(f"Issue #{issue_num} state changed from {old_state.get('state')} to {issue_data['state']}")
            any_changes = True
    ...
    if any_changes:
        ...
        # Here we could add notification logic (email, chat, etc.)
    else:
